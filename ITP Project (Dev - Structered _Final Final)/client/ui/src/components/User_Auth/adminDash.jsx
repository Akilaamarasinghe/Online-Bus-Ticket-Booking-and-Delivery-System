...
import { Modal, Form, Button, Row, Col, Container, Alert, Toast } from 'react-bootstrap';
...
function AdminDashboard() {
    ...
    const handleManageUsers = async () => {
        try {
            ...
            const admins = userData.filter(user => [1, 2, 3, 4, 5].includes(user.userEntry));
    ...
    const handleSearchChange = (event) => {
        ...
        const filteredAdmins = adminUsers.filter(user => user.name.toLowerCase().includes(query));
        const filteredCustomers = customerUsers.filter(user => user.name.toLowerCase().includes(query));
    ...
    const handleFilterChange = (event) => {
        ...
        if (roleFilter === "") {
            ...
            setFilteredUsers([...filteredAdmins, ...filteredCustomers]);
            setHeadingText(roleFilter === 'Customer' ? "Customers" : roleFilter + "s");
        }
    };
        ...
        <div className="bg-dark py-5">
            <div className="admin-container">
                <div className="admin-side-panel">
                    ...
                    <h2 style={{ fontFamily: 'Impact', textAlign: 'center', margin: '20px 0', color: 'white' }}>Admin Dashboard</h2>
                    <div className={`admin-option ${selectedOption === 'manageUsers' ? 'selected' : ''}`} onClick={handleManageUsers}>
                        <img src={profileIcon} alt="Manage Users" className="icon" />
                        <p>Manage Users</p>
                    </div>
                    <div className="admin-option" onClick={() => setShowAddAdminModal(true)}>
                        <img src={addAdminIcon} alt="Add Admin" className="icon" />
                        <p>Add Admin</p>
                    </div>
                    <div className={`admin-option ${selectedOption === 'viewFeedbacks' ? 'selected' : ''}`} onClick={handleViewFeedbacks}>
                        <img src={feedbackIcon} alt="View Feedbacks" className="icon" />
                        <p>View Feedbacks</p>
                    </div>
                    <div className={`admin-option ${selectedOption === 'viewLuggages' ? 'selected' : ''}`} onClick={handleViewLuggages}>
                        <img src={luggageIcon} alt="View Luggages" className="icon" />
                        <p>View Luggages</p>
                    </div>
                    <div className={`admin-option ${selectedOption === 'logout' ? 'selected' : ''}`} onClick={handleLogout}>
                        ...
                        <>
                            <div className="icon-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px', backgroundColor: '#343a40', borderRadius: '8px', border: '1px solid #ccc' }}>
                                <div className="search-container" style={{ display: 'flex', alignItems: 'center' }}>
                                    ...
                                    <input type="text" placeholder="Search Users" value={searchQuery} onChange={handleSearchChange} style={{ border: 'none', outline: 'none', background: 'transparent', color: '#fff', fontSize: '16px', marginLeft: '10px', padding: '5px' }} />
                                </div>
                                ...
                                <Button onClick={handleDownloadPDF} style={{ backgroundColor: 'transparent', border: 'none', outline: 'none' }}>
                                    <img src={downloadIcon} alt="Download PDF" className="icon" style={{ width: '24px', height: '24px' }} />
                                    <span style={{ color: '#fff', marginLeft: '5px', cursor: 'pointer' }}>Download User Report PDF</span>
                                </Button>
                            </div>
                                ...
                                <Row>
                                    {filteredUsers.map((user, index) => (
                                        <Col key={index} xs={12} sm={6} md={4} lg={3}>
                                            {user.userEntry !== 0 ? <AdminCard user={user} /> : <UserCard user={user} />}
                                        </Col>
                                    ))}
                                </Row>
                            </Container>
        ...
        <FeedbackList />
                ...
                >
                    <Modal.Header closeButton style={{ backgroundColor: '#343a40', borderBottom: 'none' }}>
                        <Modal.Title style={{ fontFamily: 'Impact', color: 'white' }}>Add Admin</Modal.Title>
                    </Modal.Header>
                    <Modal.Body style={{ backgroundColor: '#343a40', color: 'white' }}>
                    <Form>
    <Form.Group controlId="formName">
        <Form.Label>Name</Form.Label>
        <Form.Control type="text" placeholder="Enter name" value={newAdmin.name || ""} onChange={(e) => setNewAdmin({...newAdmin, name: e.target.value})} />
    </Form.Group>

    <Form.Group controlId="formEmail">
        <Form.Label>Email address</Form.Label>
        <Form.Control type="email" placeholder="Enter email" value={newAdmin.email || ""} onChange={(e) => setNewAdmin({...newAdmin, email: e.target.value})} />
    </Form.Group>

    <Form.Group controlId="formPhoneNumber">
        <Form.Label>Phone Number</Form.Label>
        <Form.Control type="text" placeholder="Enter phone number" value={newAdmin.phoneNumber || ""} onChange={(e) => setNewAdmin({...newAdmin, phoneNumber: e.target.value})} />
    </Form.Group>

    <Form.Group controlId="formPassword">
        <Form.Label>Password</Form.Label>
        <Form.Control type="password" placeholder="Password" value={newAdmin.password || ""} onChange={(e) => setNewAdmin({...newAdmin, password: e.target.value})} />
    </Form.Group>

    <Form.Group controlId="formUserRole">
        <Form.Label>User Role</Form.Label>
        <Form.Control as="select" value={newAdmin.adminRole || ""} onChange={(e) => setNewAdmin({...newAdmin, adminRole: e.target.value})}>
